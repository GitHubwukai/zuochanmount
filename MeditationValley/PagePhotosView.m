...
- (id)initWithFrame:(CGRect)frame withDataSource:(id<PagePhotosDataSource>)_dataSource {
    if ((self = [super initWithFrame:frame])) {
		...
		for (unsigned ii = 0; ii < kNumberOfPages; ii++) {
        ...
        pageControl.backgroundColor = [UIColor colorWithWhite:0.0 alpha:0.2];
        pageControl.bounds = CGRectMake(-100, 0, 320, 15);
        ...
        [NSTimer scheduledTimerWithTimeInterval:3 target:self selector:@selector(scrollViewDidScroll) userInfo:nil repeats:YES];
...
- (void)scrollViewDidScroll {
    ...
    [self loadScrollViewWithPage:i + 1];
    [scrollView setContentOffset:CGPointMake(scrollView.frame.size.width*i, 0) animated:YES];
	i++;
